...
echo Downloading/updating Video TTS from GitHub...
if not exist "%ROOT%web" mkdir "%ROOT%web"
for %%F in (create_video_tts_from_srt.py requirements.txt) do powershell -NoProfile -Command "Invoke-WebRequest -UseBasicParsing '%REPO%/%%F' -OutFile '%ROOT%%%F'" || goto :error
for %%F in (index.html styles.css app.js favicon.svg) do powershell -NoProfile -Command "Invoke-WebRequest -UseBasicParsing '%REPO%/web/%%F' -OutFile '%ROOT%web\%%F'" || goto :error
