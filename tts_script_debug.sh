...
# Script v2.4 para generar audio TTS con ajuste automático de velocidad
# Compatible con macOS (say) y Linux/otros (gTTS via Python)
# Congela frames solo cuando es necesario
# NUEVO: Detección automática de sistema operativo
# Uso: ./srt_to_video_v2.4.sh [archivo.srt] [video] [carpeta_audios_opcional] [--test] [--solo-audio]
...
detect_tts_method() {
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS - verificar si existe comando 'say'
    ...
    # Linux u otro sistema - verificar Python y dependencias
    if command -v python3 &> /dev/null; then
        ...
        if [ ! -f "$PYTHON_TTS_SCRIPT" ]; then
            ...
            echo -e "${YELLOW}Por favor, asegúrate de tener generate_tts.py en el mismo directorio${NC}"
            return 1
        fi
        ...
        if python3 -c "import gtts, pydub" 2>/dev/null; then
            ...
            echo -e "${YELLOW}También necesitas ffmpeg para pydub${NC}"
            return 1
        fi
    fi
    ...
    echo -e "${YELLOW}  - macOS con comando 'say', O${NC}"
    echo -e "${YELLOW}  - Python 3 con gtts y pydub instalados${NC}"
    return 1
}

# Variable global para método TTS
TTS_METHOD=""
PYTHON_TTS_SCRIPT=""
...
generate_audio_with_rate() {
    ...
    if [ "$TTS_METHOD" = "say" ]; then
        ...
        # Linux/Otro - usar script Python con gTTS
...
# Detectar método TTS disponible
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
...
# Configurar script Python si es necesario
